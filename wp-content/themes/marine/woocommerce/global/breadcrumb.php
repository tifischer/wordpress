...
$delimiter = '<i class="icons icon-angle-right"></i>';
$prepend      = '';
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		if ( get_post_type() == 'product' ) {
			...
			if ( $terms = wc_get_product_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				...
				$back_to = '<a href="' . get_term_link( $main_term->slug, 'product_cat' ) . '">' . sprintf(__('Back to %s', 'marine'), $main_term->name) . '</a>';
			}
	...
	if ($back_to) {
		echo '<div class="back-to">'.$back_to.'</div>';
	}
