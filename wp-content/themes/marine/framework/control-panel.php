					...
					<?php else: ?>
						<ul id="background-pattern">
							<?php foreach ($background_patterns as $pattern): ?>
								<li><div style="background-image: url(<?php echo get_template_directory_uri(); ?>/img/body-bg/<?php echo $pattern['value']; ?>);" data-bg="<?php echo $pattern['value']; ?>"></div></li>
							<?php endforeach; ?>
						</ul>
						<div class="clear"></div>
					<?php endif; ?>
					...
					<?php else: ?>
						<ul id="background-image">
							<?php foreach ($background_images as $background): ?>
								<li><div style="background-image: url(<?php echo ts_get_resized_image(get_template_directory_uri().'/img/body-img/'.$background,25, 25, '', '', true, true); ?>);" data-img="<?php echo $background; ?>"></div></li>
							<?php endforeach; ?>
						</ul>
						<div class="clear"></div>
					<?php endif; ?>
...
<script>
	jQuery(document).ready(function($) {
		...
		$('#panel-main-color li div').click(function() {
			...
			$(current_color_container).html('<i class="icons icon-spinner"></i>');
