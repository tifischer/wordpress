...
jQuery(document).ready(function($) {
	...
	var previewFont = function(element) {
		...
		if (jQuery(element).val().search('Proxima Nova Alt') != -1)
		{
			font = jQuery(element).val();
			
			if (jQuery.inArray(font, usedFonts) == -1)
			{
				jQuery('head').append('<style type="text/css">@font-face { font-family: "' + font + '";src: url("' + themeurl + '/font/' + font + '.otf") format("opentype");}</style>');
			}
			usedFonts[usedFonts.length] = font;
		}
	...
	jQuery('#ts-import-sample-data').click( function() {
		if (confirm(jQuery(this).data('warning'))) {
			return true;
		}
		return false;
	})
