		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
                ...
                <div class="header-menu-container">
					<p class="field-link-header">
						<label for="edit-menu-item-header-<?php echo $item_id; ?>">
							...
							<?php _e( 'Create header from this item', 'framework' ); ?>
						</label>
					</p>  
				</div>
				<!-- /Header Menu item -->
                ...
                <div class="footer-menu-container">
					<p class="field-link-footer">
						<label for="edit-menu-item-footer-<?php echo $item_id; ?>">
							<?php _e( 'Mega menu footer content', 'framework' ); ?><br />
				...
				<div class="menu-item-actions description-wide submitbox">
					...
					); ?>"><?php _e( 'Remove','framework'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
...
class ts_walker_nav_menu extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ($depth == 1 && $item -> header == 'enabled') {
			...
			$icon = '';
			if (!empty($item -> icon)) {
				$icon = '<i class="'.(strstr($item -> icon,'fa-') ? 'fa' : '').' '. $item -> icon.'"></i>';
			}
