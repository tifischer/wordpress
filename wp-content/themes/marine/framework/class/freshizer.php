...
define ('CACHE_TIME', 604800);//604800; // (60 * 60 * 24 * 7); // 7 days
...
/*----------------------------------------------------------------------------*/	
	...
	public function saveImage( $image, $path, $newDimensions = '') {
		$pinfo = pathinfo( $path );
		$ext = $pinfo['extension'];
		$return = null;
		
		$quality = 80;
		if (isset($newDimensions['dst']['w']) && $newDimensions['dst']['w'] < 200) {
			$quality = 90;
		}
		
		//saves image with default quality
		switch( $ext ) {
			case 'jpg':
...
class fImgResizer {
	...
	public function resize( fImgData $imgData ) {// stdClass $pathInfo, stdClass $imgInfo ) {
		...
		$this->_getFileSystem()->saveImage( $imageNew, $imgData->new->path, $newDimensions );
		...
		if (defined('RETINA_SUPPORT') && RETINA_SUPPORT == true)
		{
			...
			$pinfo = pathinfo( $imgData->new->path );
			$ext = $pinfo['extension'];
			$retina_path = $pinfo['dirname'].'/'.$pinfo['filename'].'@2x.'.$pinfo['extension'];

			$this->_getFileSystem()->saveImage( $imageNew, $retina_path, $newDimensions );
			imagedestroy($imageNew);
		}
