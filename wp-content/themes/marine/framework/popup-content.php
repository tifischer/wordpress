...
jQuery(document).ready(function($) {
	...
	var formatIconDropdownShortcode = function(item) {
		...
		if (item.text.indexOf("icon-") >= 0 || item.text.indexOf("icomoon-") >= 0) {
			...
			return '<i class="icons ' + item.text + '"></i> ' + item_text;
		}
		return item.text;
	}
			...
			for (var key in level.fields) {
						...
						html += '<div class="format-setting type-text has-desc">';
							...
							html += '<div class="format-setting-inner">';
								html += '<div class="option-tree-ui-upload-parent">';
									...
									html += '<a href="javascript:void(0);" class="ot_upload_media option-tree-ui-button button button-primary light" rel="4" title="Add Media"><span class="icon ot-icon-plus-sign"></span>Add Media</a>';
								html += '</div>';
							html += '</div>';
						html += '</div>';
						break;
				...
				switch (field.type)
				{
					case 'wp_editor':
						...
						tinyMCE.execCommand( "mceAddEditor", true, current_key );
	...
	jQuery('#insert-shortcode').click(function(){
		...
		<?php else: ?>
			window.tinymce.activeEditor.execCommand('mceInsertContent', false, shortocdeFinalString);
		<?php endif; ?>
		tb_remove();
