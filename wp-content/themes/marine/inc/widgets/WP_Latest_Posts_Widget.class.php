...
class WP_Latest_Posts_Widget extends WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		extract($args);
		echo $before_widget;
		$title = apply_filters('widget_title', empty($instance['title']) ? __( 'Latest Posts', "framework" ) : $instance['title'], $instance, $this->id_base);
		if ( empty( $instance['number'] ) || ! $number = absint( $instance['number'] ) )
		{
			$number = 10;
		}
		$r = new WP_Query( array('orderby' => 'date', 'order' => 'DESC', 'posts_per_page' => $number, 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true)  );
		if ($r->have_posts()) : ?>
			...
			<?php $i = 1;?>
				...
				<?php  while ($r->have_posts()) : $r->the_post(); ?>
					...
					<div class="blog-post">
						 <span class="post-meta"><?php the_time(get_option('date_format')); ?> | <?php the_author();?></span>
						 <a class="post-title" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>" href="<?php the_permalink() ?>"><?php if ( get_the_title() ) echo ts_get_shortened_string_by_letters(get_the_title(),14); else the_ID(); ?></a>
                       <p><?php echo ts_get_the_excerpt_theme(10);?></p>
