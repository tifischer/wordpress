...
function ts_heading_func($atts, $content = null )
{
	...
	switch ($align) {
		case 'center':
		case 'right':
			$align_class = 'class="align-'.$align.' '.ts_get_animation_class($animation).'"';
			break;
		default:
			$align_class = ts_get_animation_class($animation,true);
	}
	...
	return '<h'.$type.' '.$align_class.'  '.$style.'  data-animation="'.$animation.'">'.do_shortcode($content).'</h'.$type.'>';
}
